...
public final class RecordTypeAdapterFactory implements TypeAdapterFactory {
  ...
  private BoundComponent createBoundComponent(
      final int index,
      final Gson context,
    ...
    if (adapter != null) {
      typeAdapter =
          jsonAdapterFactory.getTypeAdapter(
              constructorConstructor, context, fieldToken, adapter, true);
    }
  ...
  private Map<String, BoundComponent> getComponents(
      final Gson context, final TypeToken<?> type, final Descriptor descriptor) {
    ...
    for (int i = 0; i < fieldCount; ++i) {
      ...
      for (int j = 0, jj = fieldNames.length; j < jj; ++j) {
        ...
        final BoundComponent boundComponent =
        ...
        if (result.put(name, boundComponent) != null) {
          throw new IllegalArgumentException(
              type.getType() + " declares multiple JSON fields named " + name);
        }
      }
    }
  ...
  static final class BoundComponent {
    ...
    BoundComponent(
      ...
      this.adapterOut =
          wrap ? new TypeAdapterRuntimeTypeWrapper(context, typeAdapter, fieldType) : typeAdapter;
    ...
    void write(final JsonWriter writer, final Object object) throws IOException {
      if (serialize) {
        try {
          ...
          throw new JsonIOException(
              "Accessor method '" + className + "#" + name + "()' threw exception", error);
        }
      } else {
        writer.nullValue();
      }
